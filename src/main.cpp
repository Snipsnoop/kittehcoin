...
uint256 hashGenesisBlock("0x984b30fc9bb5e5ff424ad7f4ec1930538a7b14a2d93e58ad7976c23154ea4a76");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KittehCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KittehCoin Signed Message:\n";
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KittehCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x16a9688c3fc8b4f0fcb0ea7453dbf0de698bc4e45f7bf111cf4cf149505d77ee");
    }
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // KittehCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KittehCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KittehCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KittehCoinMiner(CWallet *pwallet)
{
    printf("KittehCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kittehcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KittehCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KittehCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KittehCoinMiner, pwallet));
}
